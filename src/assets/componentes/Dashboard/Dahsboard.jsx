...
function Dashboard() {
  ...
  const handleSelectChange = (e, colmenaId) => {
    setSelectValues({ ...selectValues, [colmenaId]: e.target.value });
    switch (e.target.value) {
      case 'editar': navigate(`/EditColmena/${colmenaId}`); break;
      case 'monitoreo': navigate(`/Monitoreo/${colmenaId}`); break;
      case 'recoleccion': navigate(`/Recoleccion/${colmenaId}`); break;
      case 'lista_monitoreo': navigate('/List_Monitoreo'); break;
      case 'lista_recoleccion': navigate('/List_Recoleccion'); break;
  ...
  const closePopup = () => {
    if (selectedColmenaId) setSelectValues({ ...selectValues, [selectedColmenaId]: '' });
    setShowPopup(false);
    setShowQR(false);
    setQrData(null);
    setTimeout(() => setSelectedColmenaId(null), 400);
  };
  ...
  const handleQRAction = async (colmenaId, action = 'download') => {
    try {
      ...
      if (!response.ok) throw new Error('Error al obtener los detalles de la colmena');
      const hives = await response.json();
      const selectedHive = hives.find(hive => hive.id === colmenaId);
      if (!selectedHive || !selectedHive.qr_code) throw new Error('No se encontró el código QR');
  ...
  const generatePDF = () => {
    ...
    data.forEach((colmena, index) => {
      ...
      // Fetch history for this colmena (simplified for PDF, ideally fetch this data beforehand)
      doc.text('Historial:', 10, yOffset);
      yOffset += 5;

      const fetchHistoryForPDF = async () => {
      ...
      fetchHistoryForPDF(); // Note: This is async and might not work as expected in PDF generation without proper handling
  ...
  const modalStyles = {
    ...
    modalContent: { transform: showPopup ? 'scale(1)' : 'scale(0.8)', transition: 'transform 0.3s ease' },
  ...
  styleElement.textContent = `
    ...
    .honey-button { transition: all 0.2s ease; position: relative; overflow: hidden; }
    .honey-button:after { content: ''; position: absolute; width: 100%; height: 100%; top: 0; left: 0; background-image: radial-gradient(circle, transparent 10%, rgba(255,255,0,0.1) 10.01%); transform: scale(10, 10); opacity: 0; transition: transform 0.5s, opacity 0.5s; }
    .honey-button:active:after { transform: scale(0, 0); opacity: 0.3; transition: 0s; }
    @media (max-width: 576px) { .bee-card .row { flex-direction: column; align-items: stretch; } .bee-card img { max-height: 150px; margin: 0 auto; } .bee-card .col-sm-5, .bee-card .col-sm-3 { text-align: center; margin-top: 10px; } .form-select { width: 100%; margin: 0 auto; } }
  `;
  ...
  return (
    <div className="d-flex flex-column min-vh-100">
            ...
            <div className="d-flex flex-column gap-3">
              {error ? (
                ...
                data.map((colmena, index) => (
                  <div key={colmena.id}>
                    <div className="card rounded p-3 mx-2 mx-md-3 bee-card position-relative" style={modalStyles.hiveCard}>
                      <div className="row g-0 align-items-center">
                        <div className="col-12 col-sm-4 mb-3 mb-sm-0">
                          <div id={`image-flip-${colmena.id}`} style={{ transition: 'transform 0.6s', transformStyle: 'preserve-3d', position: 'relative', width: '100%', height: '150px' }}>
                            <button className="btn btn-sm btn-warning position-absolute" style={{ top: '5px', left: '5px', zIndex: 10, padding: '3px 8px', fontSize: '0.8rem', opacity: 0.9 }} onClick={(e) => {
                              e.stopPropagation();
                              const imageContainer = document.getElementById(`image-flip-${colmena.id}`);
                              if (imageContainer) imageContainer.style.transform = imageContainer.style.transform === 'rotateY(180deg)' ? 'rotateY(0deg)' : 'rotateY(180deg)';
                            }}>
                              <i className="bi bi-arrow-repeat"></i>
                            </button>
                            <div style={{ backfaceVisibility: 'hidden', position: 'absolute', width: '100%', height: '100%' }}>
                              <img src={imagenes[index % imagenes.length]} alt="Imagen de la colmena" className="img-fluid rounded" style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.3s ease' }} onMouseOver={(e) => (e.target.style.transform = 'scale(1.05)')} onMouseOut={(e) => (e.target.style.transform = 'scale(1)')} />
                            </div>
                            <div style={{ backfaceVisibility: 'hidden', position: 'absolute', width: '100%', height: '100%', transform: 'rotateY(180deg)', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff' }}>
                              {colmena.qr_code ? (
                                <img src={`data:image/png;base64,${colmena.qr_code}`} alt={`QR Code for hive ${colmena.id}`} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
                              ) : (
                        ...
                        <div className="col-12 col-sm-5 text-center text-sm-start mb-3 mb-sm-0">
                          ...
                          <p className="mb-0 ms-0 ms-sm-3">
                            <span className={`badge ${colmena.status === 'Active' ? 'bg-success' : 'bg-danger'}`} style={modalStyles.statusBadge}>
                              {colmena.status === 'Active' ? 'Activa' : 'Inactiva'}
                            </span>
                          </p>
                        </div>
                        <div className="col-12 col-sm-3 text-center">
                          <select className="form-select form-select-sm bg-warning-subtle border-warning-subtle" onChange={(e) => handleSelectChange(e, colmena.id)} value={selectValues[colmena.id] || ''}>
      ...
      <Footer />
      <div className={`modal fade ${showPopup ? 'show' : ''}`} id="colmenaModal" tabIndex="-1" aria-labelledby="colmenaModalLabel" aria-hidden="true" style={{ display: showPopup ? 'block' : 'none', backgroundColor: 'rgba(0, 0, 0, 0.5)', ...modalStyles[modalAnimation] }}>
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content" style={modalStyles.modalContent}>
            <div className="modal-header border-0 pb-0">
              <img src={logo} alt="Logo" style={{ height: '35px', animation: showPopup ? 'spin 1s ease-in-out' : 'none' }} className="mx-auto" />
                  ...
                  return (
                    <div style={modalStyles.detailsAnimation}>
                      <h5 className="modal-title text-center mb-4" id="colmenaModalLabel">
                        <span style={{ display: 'inline-block', animation: 'fadeIn 0.5s ease-out forwards' }}>Información de la colmena</span>
                      </h5>
                      <div className="row">
                        <div className="col-12 col-md-5 text-center mb-4 mb-md-0">
                          <img src={imagen1} alt="Imagen de colmena" className="rounded-circle border border-3 border-secondary" style={{ width: '150px', height: '150px', objectFit: 'cover', animation: 'fadeIn 0.5s ease-out forwards', transition: 'transform 0.3s ease' }} onMouseOver={(e) => (e.target.style.transform = 'scale(1.05) rotate(5deg)')} onMouseOut={(e) => (e.target.style.transform = 'scale(1) rotate(0deg)')} />
                          <div className="d-flex flex-column gap-2 mt-3">
                            <button className="btn btn-warning honey-button" onClick={() => navigate(`/EditColmena/${selectedColmena.id}`)} style={{ animation: 'fadeIn 0.5s ease-out forwards', animationDelay: '0.1s', opacity: 0 }}>Editar</button>
                            <button className="btn btn-warning honey-button" onClick={() => handleQRAction(selectedColmena.id, 'download')} style={{ animation: 'fadeIn 0.5s ease-out forwards', animationDelay: '0.2s', opacity: 0 }}>Descargar QR</button>
                            <button className="btn btn-warning honey-button" onClick={() => handleQRAction(selectedColmena.id, 'view')} style={{ animation: 'fadeIn 0.5s ease-out forwards', animationDelay: '0.3s', opacity: 0 }}>{showQR ? 'Ocultar QR' : 'Ver QR'}</button>
                            <button className={`btn honey-button ${selectedColmena.status === 'Active' ? 'btn-danger' : 'btn-success'}`} onClick={() => handleChangeHiveState(selectedColmena.id)} disabled={statusUpdating} style={{ animation: 'fadeIn 0.5s ease-out forwards', animationDelay: '0.4s', opacity: 0 }}>
                              {statusUpdating ? <span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span> : null}
                          ...
                          <div style={modalStyles.qrAnimation}>
                            {qrData && (
                              <div className="mb-3 text-center">
                                <h6>Código QR:</h6>
                                <img src={`data:image/png;base64,${qrData}`} alt="Código QR de la colmena" style={{ maxWidth: '200px', maxHeight: '200px' }} />
                              </div>
                            )}
                          </div>
                          {[
                            ...
                            <p key={index} style={{ animation: 'fadeIn 0.5s ease-out forwards', animationDelay: `${0.1 + index * 0.05}s`, opacity: 0, transform: 'translateY(10px)' }}>
